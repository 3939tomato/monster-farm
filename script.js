...
// ==========================================
// 拡張パック：UI完全修復・睡眠回復システム
// ==========================================
(function() {
    // 1. 睡眠中（💤）のHP回復システム
    // 1秒間に約20のペースで目に見えて回復させます
    setInterval(() => {
        if (typeof monsters === 'undefined') return;
        monsters.forEach(m => {
            if (m.emotion === "💤" && !m.isDead) {
                m.hp = Math.min(m.hpMax, m.hp + 2.0);
            }
        });
    }, 100);

    // 2. ターゲットモニター（ステータスUI）の完全修復＆拡張
    // 既存のメインループが吐き出す不完全なUIを監視し、瞬時に正しいものに書き換えます
    const statsEl = document.getElementById('targetStats');
    if (!statsEl) return;

    let isUpdatingUI = false;
    const uiObserver = new MutationObserver(() => {
        // 無限ループ防止と、ターゲット未選択時のエラーガード
        if (isUpdatingUI || typeof selectedObject === 'undefined' || !selectedObject) return;
        
        try {
            isUpdatingUI = true;
            let html = "";

            // species（種族名）を持っていればモンスター、それ以外はエサ/環境物として判定
            if (selectedObject.species !== undefined) {
                
                // モンスターの場合：名前の横に感情アイコンを追加
                const em = selectedObject.emotion ? `<span style="font-size:1.5em; margin-left:8px;">${selectedObject.emotion}</span>` : "";
                // 睡眠中はHPの数値を緑色で強調
                const hpStyle = selectedObject.emotion === '💤' ? 'color:#4CAF50; font-weight:bold;' : '';
                
                html = `
                    <div style="border-bottom:1px solid #555; margin-bottom:5px; padding-bottom:2px;">
                        <b style="font-size:1.2em;">${selectedObject.species}</b> ${em} <span style="font-size:0.9em;">[Lv.${selectedObject.level}]</span>
                    </div>
                    性格: ${selectedObject.personality || "不明"}<br>
                    HP: <span style="${hpStyle}">${Math.floor(selectedObject.hp)}</span> / ${selectedObject.hpMax}<br>
                    空腹: ${Math.floor(selectedObject.hunger)}%<br>
                    パワー: ${selectedObject.power} / スピード: ${Math.floor(selectedObject.speedVal * 10)}<br>
                    耐性: 🔥${selectedObject.heatResist} ❄️${selectedObject.coldResist}
                `;
            } else {
                // 食べ物・環境オブジェクトの場合：消えていた表記を復活
                const tName = selectedObject.type || "オブジェクト";
                const isCorpse = tName === 'corpse' || selectedObject.isCorpse;
                const icon = tName === 'fish' ? '🐟' : tName === 'fruit' ? '🍎' : tName === 'mushroom' ? '🍄' : isCorpse ? '🦴' : '🌳';
                const statusText = isCorpse ? "風化待ち（なきがら）" : "フィールドアイテム";

                html = `
                    <div style="border-bottom:1px solid #555; margin-bottom:5px; padding-bottom:2px;">
                        <b style="font-size:1.2em;">${icon} ${tName.toUpperCase()}</b>
                    </div>
                    状態: ${statusText}<br>
                    座標: X:${Math.floor(selectedObject.x)} Y:${Math.floor(selectedObject.y)}
                `;

                // プレビュー画面（左上の小窓）にアイテムのアイコンを描画
                const tCtx = document.getElementById('targetPreview').getContext('2d');
                if (tCtx) {
                    tCtx.clearRect(0, 0, 32, 32);
                    tCtx.font = "24px sans-serif";
                    tCtx.fillText(icon, 4, 26);
                }
            }
